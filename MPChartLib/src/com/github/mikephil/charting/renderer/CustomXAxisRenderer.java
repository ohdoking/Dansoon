...
public class CustomXAxisRenderer extends CustomAxisRenderer {

    public CustomXAxisRenderer(ViewPortHandler viewPortHandler, CustomXAxis xAxis, Transformer trans) {
    ...
    protected CustomXAxis mXAxis;

    public void computeAxis(float xValMaximumLength, List<Bitmap> xValues) {
    ...
    public void renderAxisLabels(Canvas c) {
        ...
        if (mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.TOP) {

            drawLabels(c, mViewPortHandler.contentTop() - yoffset,
                    new PointF(0.5f, 1.0f));

        } else if (mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.TOP_INSIDE) {

            drawLabels(c, mViewPortHandler.contentTop() + yoffset + mXAxis.mLabelRotatedHeight,
                    new PointF(0.5f, 1.0f));

        } else if (mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.BOTTOM) {

            drawLabels(c, mViewPortHandler.contentBottom() + yoffset,
                    new PointF(0.5f, 0.0f));

        } else if (mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.BOTTOM_INSIDE) {
    ...
    public void renderAxisLine(Canvas c) {
        ...
        if (mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.TOP
                || mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.TOP_INSIDE
                || mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.BOTH_SIDED) {
            c.drawLine(mViewPortHandler.contentLeft(),
                    mViewPortHandler.contentTop(), mViewPortHandler.contentRight(),
                    mViewPortHandler.contentTop(), mAxisLinePaint);
        }

        if (mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.BOTTOM
                || mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.BOTTOM_INSIDE
                || mXAxis.getPosition() == CustomXAxis.CustomXAxisPosition.BOTH_SIDED) {
            c.drawLine(mViewPortHandler.contentLeft(),
    ...
    protected void drawLabels(Canvas c, float pos, PointF anchor) {
        ...
        for (int i = mMinX; i <= mMaxX; i += mXAxis.mAxisLabelModulus) {
            ...
            if (mViewPortHandler.isInBoundsX(position[0])) {
                ...
                drawLabel(c, label.toString(), i, position[0], pos, anchor, labelRotationAngleDegrees);
            }
        }
    }
    ...
    protected void drawLabelBitmap(Canvas c, Bitmap label, int xIndex, float x, float y, PointF anchor, float angleDegrees) {
        /*String formattedLabel = mXAxis.getValueFormatter().getXValue(label, xIndex, mViewPortHandler);
        Utils.drawText(c, formattedLabel, x, y, mAxisLabelPaint, anchor, angleDegrees);*/
        c.drawBitmap(label,x, y, mAxisLabelPaint);
    }
